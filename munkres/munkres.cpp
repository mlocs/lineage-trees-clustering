...
Matrix<double> 
Munkres::solveForMaxWeightMatching(int nrows, int ncols, double* d_array) {
	Matrix<double> matrix(nrows, ncols);
	for (int i = 0; i < nrows; i++) {
		for (int j = 0; j < ncols; j++) {
			//matrix(i,j) = (d_array[i*ncols + j] == 0) ? 0 : 1.0 / (d_array[i*ncols + j]) ;
			matrix(i,j) = MEGABIG_NUM - d_array[i*ncols + j];
		}
	}
...
double 
Munkres::maxWeightMatching(double* data_array, int nrows, int ncols) {
	Matrix<double> matrix = solveForMaxWeightMatching( nrows, ncols, data_array);
	double val = 0.0;
	for (int i = 0; i < matrix.rows(); i++) {
		for (int j = 0; j < matrix.columns(); j ++) {
			if (matrix(i,j) == 0) val += data_array[i*ncols + j];
		}
	}
